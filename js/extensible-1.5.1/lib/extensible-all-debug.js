...
Ext.define('Extensible.calendar.template.BoxLayout', {
	...
	*/
                ...
                '<div id="{[this.id]}-wk-{[xindex-1]}" class="ext-cal-wk-ct" style="top:{[this.getRowTop(xindex, xcount)]}%; height:{[this.getRowHeight(xcount)]}%;">',
                    ...
                    '<table class="ext-cal-evt-tbl" cellpadding="0" cellspacing="0">',
                        '<tbody>',
                            '<tr>',
                                '<tpl for=".">',
                                    '<td id="{[this.id]}-ev-day-{date:date("Ymd")}-{calendarId}" class="{titleCls}"><p>{title}</p><div style="font-weight: bold;padding: 15px 5px 5px 5px;">{calendarName}</div></td>',
                                '</tpl>',
                            '</tr>',
                        '</tbody>',
                    '</table>',
	...
	applyTemplate: function (o)
	{
		...
		var i = 0, calendarNames = [], calendarIds=[];
		for (; i < calendarStore.getCount(); i++)
		{
			calendarNames[i] = calendarStore.data.items[i].data.Title;
			calendarIds[i] = calendarStore.data.items[i].data.CalendarId;
		}
		for (; w < this.weekCount || this.weekCount == -1; w++)
		{
			...
			weeks[w] = [];
			//this.dayCount = 6;
			for (var d = 0; d < calendarNames.length; d++)
			{
...
//				{
					...
					{
						title = Ext.Date.format(dt, 'D, j M'/*this.dayCount == 1 ? this.singleDayDateFormat :
...
//				}
				weeks[w].push({
					calendarName: calendarNames[d],
					calendarId: calendarIds[d],
					title: title,
	...
	getTodayText: function ()
	{
		...
		if (this.dayCount == 1)
		{
			return Ext.Date.format(new Date(), 'D, j M'/*this.singleDayDateFormat*/) + separator + todayText + timeText;
		}
...
Ext.define('Extensible.calendar.template.DayHeader', {
	...
	applyTemplate: function (o)
	{
		...
		templateConfig.calendarStore= this.calendarStore;
		if (Ext.getVersion().isLessThan('4.1'))
		{
...
Ext.define('Extensible.calendar.template.DayBody', {
                        ...
                        '<tpl for="calendarIds">',
                            '<td class="ext-cal-day-col">',
                                '<div class="ext-cal-day-col-inner">',
                                    '<div id="{[this.id]}-day-col-{.}-{parent.day:date("Ymd")}" class="ext-cal-day-col-gutter" style="height:{parent.dayHeight}px;"></div>',
                                '</div>',
                            '</td>',
                        '</tpl>',
                    '</tr>',
	...
	applyTemplate: function (o)
	{
		...
		templateConfig = {
			day: Ext.Date.clone(o.viewStart),
			days: days,
...
Ext.define('Extensible.calendar.data.MemoryCalendarStore', {
	...
	listeners:
	{
		...
		{
		    debugger;
			var cstore = Ext.getStore('CalendarLocalStorage');
			for (var i = 0; i < records.length; i++)
			{
				cstore.add(records[i].data);
			}
			cstore.sync();
		},
		...
		{
			var cstore = Ext.getStore('CalendarLocalStorage');
			var crecord = cstore.findRecord('CalendarId', record.data.CalendarId);
			if (crecord && modifiedFieldNames)
			{
				for (var i = 0; i < modifiedFieldNames.length; i++)
				{
					crecord.set(modifiedFieldNames[i], record.data[modifiedFieldNames[i]]);
				}
			}
			cstore.sync();
		},
		...
		{
			var cstore = Ext.getStore('CalendarLocalStorage'),
			crecord = cstore.findRecord('CalendarId', record.data.CalendarId);
			cstore.remove(crecord);
			cstore.sync();
		}
	},
	//autoLoad: true,
 ...
 */
Ext.define('Extensible.calendar.gadget.CalendarListEditDialog',
{
	extend: 'Ext.window.Window',
	alias: 'widget.extensible.calendarListEditDialog',
	title: 'Calendario',
	modal: true,
	layout: 'fit',
	width: 300,
		...
		[
			{
				...
				[
                        ...
                        [                           
					        {
					            ...
					            handler: function ()
					            {
					                var menu = Ext.create('Extensible.calendar.gadget.CalendarListMenu');
					                menu.on('colorchange', function (menu, id, colorId, origColorId) {  this.setValue(colorId); }, this.up('form').down('[name=ColorId]'));
					                menu.on('hide', function () { this.destroy() });
					                menu.show();
					            }
					        },
                            {
                    ...
                    },
					{
						...
						hidden: true,
						name: 'ColorId',
						//value: rec ? rec.data.ColorId : 20,
						allowBlank: false,
						listeners:
                        {
                            change: function(feild, newValue, oldValue)
                            {
                                var container = this.up().down('[itemId = colorPreview]');
                                container.removeCls('x-cal-' + oldValue + '-ad');
                                container.addCls('x-cal-' + newValue + '-ad');
                            },
                            afterrender: function()
                            {
                                this.setValue(rec ? rec.data.ColorId : 18);
                            }
                        }
					}
				],
						...
						handler: function ()
						{
						    ...
						    if (form.isValid())
						    {
						        if (rec) {
						            ...
						            values.id = values.CalendarId = Ext.id('', '1');
						            view.store.add(values);
						        }
						        view.refresh();
			...
			[
				{
					...
					handler: function ()
					{
						var dialog = Ext.widget('extensible.calendarListEditDialog', { view: this.up('panel') });
						dialog.show();
					}
				}
			]
			...
			this.tpl = !(Ext.isIE || Ext.isOpera) ?
                Ext.create('Ext.XTemplate',
                    '<ul class="x-unselectable"><tpl for=".">',
                        '<li id="{cmpId}" class="ext-cal-evr {colorCls} {hiddenCls}" style="height: 30px; font-size:18px;">{title}<em style="height: 28px;display: block;">&#160;</em></li>',
                    '</tpl></ul>'
                )
                : Ext.create('Ext.XTemplate',
                    '<ul class="x-unselectable"><tpl for=".">',
                        '<li id="{cmpId}" class="ext-cal-evo {colorCls} {hiddenCls}" style="height: 30px; font-size:18px;">',
                            '<div class="ext-cal-evm" style="height: 100%;">',
                                '<div class="ext-cal-evi">{title}<em style="height: 28px;display: block; width: 10px;">&#160;</em></div>',
                            '</div>',
                        '</li>',
                    '</tpl></ul>'
                );
	...
	*/
	...
	onClick: function (e, t)
	{
		...
		else if (el = e.getTarget('li', 3, true))
		{
		    ...
		    debugger;
			var rec = this.store.findRecord(Extensible.calendar.data.CalendarMappings.CalendarId.name, this.getCalendarId(el));
			dialog = Ext.widget('extensible.calendarListEditDialog', { rec: rec, view: this });
			dialog.show();
		}
	},
...
Ext.define('Extensible.calendar.form.EventWindow', {
	...
	*/
	show: function (o, animateTarget, el, calendarRecord)
	{
		...
		if (calendarRecord)
		{
		    var calendarCombo = this.down('[itemId=' + this.id + '-calendar' + ']');
		    calendarCombo.setValue(calendarRecord.data.CalendarId);
		}
		return this;
	},
...
Ext.define('Extensible.calendar.view.AbstractCalendar', {
	...
	*/
	showEventEditor: function (rec, animateTarget, calendarRecord)
	{
	    this.getEventEditor().show(rec, animateTarget, this, calendarRecord);
		return this;
	},
	...
	*/
	...
	onDayClick: function (dt, ad, el, rec)
	{
		if (this.readOnly === true)
		{
			return;
		...
		if (this.fireEvent('dayclick', this, Ext.Date.clone(dt), ad, el) !== false)
		{
			...
			this.showEventEditor(data, el, rec);
		}
	},
	...
	*/
	...
	renderItems: function ()
	{
		...
		for (; day < 1; day++)
		{
			...
			for (; ev < ct; ev++)
			{
				...
				if (this.calendarStore.findRecord('CalendarId', item.CalendarId))
				{
					evts.push({
		...
		for (; i < l; i++)
		{
			...
			for (j = 0; j < l; j++)
			{
				if (i == j) continue;
				evt2 = evts[j].data;
				if (this.isOverlapping(evt, evt2) && evt.CalendarId == evt2.CalendarId)
				{
		...
		for (i = 0; i < l; i++)
		{
			...
			var markup = this.getEventTemplate().apply(evt),
                target = this.id + '-day-col-' + evts[i].data.CalendarId + '-' + Ext.Date.format(evts[0].date, 'Ymd');
	...
	getDayAt: function (x, y)
	{
		var sel = '.ext-cal-body-ct',
            ...
            dt = Extensible.Date.add(this.viewStart, { days: 0, minutes: mins, hours: this.viewStartHour }),
            el = this.getDayEl(dt),
            timeX = x;
		if (el)
		{
	...
	onClick: function (e, t)
	{
		...
		if (day && day.date)
		{
		    var rec = this.calendarStore.getAt(day.calendarIndex);
		    this.onDayClick(day.date, false, null, rec);
		}
	}
});             /**
	...
	initComponent: function ()
	{
		...
		if (this.showNavToday)
		{
			this.tbar.items.push({
				id: this.id + '-tb-today', height: 30, width: 50, text: this.todayText, handler: this.onTodayClick, scope: this
			});
		}
		if (this.showNavNextPrev)
		{
			this.tbar.items.push({ id: this.id + '-tb-prev', height: 30, width: 35, padding: 0, handler: this.onPrevClick, scope: this, iconCls: 'prev-icon' });
			this.tbar.items.push({ id: this.id + '-tb-next', height: 30, width: 35, padding: 0, handler: this.onNextClick, scope: this, iconCls: 'next-icon' });
		}
		if (this.showNavJump)
		{
			this.tbar.items.push(this.jumpToText);
			this.tbar.items.push({ id: this.id + '-tb-jump-dt', editable: false, xtype: 'datefield', width: 120, showToday: false });
		...
		{
			this.tbar.items.push({
				id: this.id + '-tb-day', text: this.dayText, hidden: true, handler: this.onDayNavClick, scope: this, toggleGroup: this.id + '-tb-views'
			});
			this.viewCount++;
		}
